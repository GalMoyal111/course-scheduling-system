...
export function DataProvider({ children }) {
  ...
  const [currentTimetableMetadata, setCurrentTimetableMetadata] =
    useState(null);
  ...
  // Fetch courses if needed. This is used to populate the list of courses for scheduling and display.
  const fetchCoursesIfNeeded = useCallback(
    async (caller = "Unknown", forceFetch = false) => {
  ...
  // Fetch lessons if needed. This is used to populate the list of lessons for scheduling and display.
  const fetchLessonsIfNeeded = useCallback(
    async (caller = "Unknown", forceFetch = false) => {
  ...
  // Fetch lecturers if needed. This is used to populate the list of lecturers for scheduling and display.
  const fetchLecturersIfNeeded = useCallback(
    async (caller = "Unknown", forceFetch = false) => {
      if (isFetching.lecturers) return;

      if (!forceFetch && isCacheValid(lecturersTimestamp)) {
        console.log(
          `[Cache] Lecturers are fresh, skipping fetch for: ${caller}`,
        );
        return;
      }
  ...
  // Fetch classrooms if needed. This is used to populate the list of available classrooms for scheduling.
  const fetchClassroomsIfNeeded = useCallback(
    async (caller = "Unknown", forceFetch = false) => {
      if (isFetching.classrooms) return;

      if (!forceFetch && isCacheValid(classroomsTimestamp)) {
        console.log(
          `[Cache] Classrooms are fresh, skipping fetch for: ${caller}`,
        );
        return;
      }
    ...
    [
      ...
      classroomsTimestamp,
      isCacheValid,
      isFetching.classrooms,
    ],
  );

  // Fetch timetable history if needed. This is used to populate the user's history of generated timetables.
  ...
  // Fetch clusters if needed. This is used to populate the list of clusters for scheduling and display.
  const fetchClustersIfNeeded = useCallback(
    async (caller = "Unknown") => {
      if (isFetching.clusters) return;

      if (isCacheValid(clustersTimestamp)) {
        console.log(
          `[Cache] Clusters are fresh, skipping fetch for: ${caller}`,
        );
        return;
      }
  ...
  // Fetch system blocked slots if needed. This is used to determine which time slots are unavailable for scheduling due to system constraints (e.g., maintenance, holidays).
  const fetchSystemBlockedSlotsIfNeeded = useCallback(
    async (caller = "Unknown", forceFetch = false) => {
      if (isFetching.systemBlockedSlots) return;

      if (!forceFetch && isCacheValid(systemBlockedSlotsTimestamp)) {
        console.log(
          `[Cache] System blocked slots are fresh, skipping fetch for: ${caller}`,
        );
        return;
      }
  ...
  // Fetch classroom size settings if needed. This is used to determine the required capacities for different types of classrooms when scheduling.
  const fetchClassroomSizeSettingsIfNeeded = useCallback(
    async (caller = "Unknown", forceFetch = false) => {
      if (isFetching.classroomSizeSettings) return;

      if (!forceFetch && isCacheValid(classroomSizeSettingsTimestamp)) {
        console.log(
          `[Cache] Classroom size settings are fresh, skipping fetch for: ${caller}`,
        );
        return;
      }
    ...
    [
      ...
      isCacheValid,
      isFetching.classroomSizeSettings,
    ],
  );

  // Load a specific timetable from history. This is used when a user wants to view or restore a previously generated timetable from their history.
  const loadTimetableFromHistory = useCallback(
    async (id, caller = "Unknown") => {
      try {
